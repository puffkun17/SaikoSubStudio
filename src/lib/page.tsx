...
export default function Home() {
  const { workflowStep, initializeLibrary, logs, tasks } = useStudioStore();
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (tasks.length > 0 && workflowStep > 1) {
        e.preventDefault();
        e.returnValue = '离开此页面将丢失任务信息';
        return '离开此页面将丢失任务信息';
      }
    };
  ...
  return (
    <main className="flex-1 w-full h-full bg-[#050507] text-white overflow-hidden flex flex-col font-sans relative">
      ...
      {/* Global Toast Logs System (Sonner Style Top-Center) */}
