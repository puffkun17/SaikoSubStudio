...
export const useStudioStore = create<StudioState>((set, get) => ({
  ...
  updateSubtitleText: (index, text) => set(state => {
    if (!state.processedSubs) return {};
    const updated = state.processedSubs.map(s => 
      s.index === index ? { ...s, text } : s
    );
    return { processedSubs: updated };
  }),
  ...
  searchTmdbManual: async (query, type, year) => {
    ...
    try {
      ...
      scored.sort((a: { item: any; score: number }, b: { item: any; score: number }) => b.score - a.score || (b.item.popularity || 0) - (a.item.popularity || 0));
      const sortedResults = scored.map((s: { item: any; score: number }) => s.item).slice(0, 10);
  ...
  selectTmdbSuggestion: async (s) => {
    ...
    try {
      ...
      // Fetch images list (stills if it is a TV episode, backdrops if it is a movie)
      let backdrops: string[] = [];
      try {
        let imagesUrl = `https://api.themoviedb.org/3/${type}/${s.id}/images?api_key=${tmdbApiKey}`;
        let hasEpisodeStills = false;
        ...
        if (imgRes.ok) {
          ...
          if (list.length > 0) {
            backdrops = list.map((img: any) => `https://image.tmdb.org/t/p/original${img.file_path}`);
          }
        }
      } catch (e) {
      ...
      // Fallback to suggestion backdrop if no images list could be fetched
      if (backdrops.length === 0 && s.backdrop_path) {
        backdrops = [`https://image.tmdb.org/t/p/original${s.backdrop_path}`];
      }

      // Randomly select one, skipping the top 3 default promo images if possible to ensure CINE/immersion feel
      let chosenBackdrop = backdrops[0] || null;
    ...
    set(state => {
      const nextTasks = state.tasks.map(t => {
        if (t.id === taskId) {
          ...
          // Dynamically adjust isBilingualSingle and clear potential conflicts
          if (trackKey === 'zh') {
            updated.isBilingualSingle = selectedFile?.lang === 'bilingual';
            if (updated.isBilingualSingle) {
              updated.en = null; // Clear English to avoid duplication when single bilingual is used
            }
          } else if (trackKey === 'en' && selectedFile) {
            updated.isBilingualSingle = false; // Cannot be single bilingual if secondary english is bound
          }
  ...
  processFiles: (newFiles) => {
    set(state => {
      ...
      // Optimize bindings for all tasks deterministically
      const optimizeTaskBindings = (task: TaskPair) => {
        const files = task.files;
        
        // 1. Separate commentary
        ...
        // Helper to select best file from a list (prefer .ass, then larger size)
        ...
        if (bestZh && bestEn) {
          // Priority A: Monolingual Chinese + Monolingual English for alignment
          task.zh = bestZh;
          task.en = bestEn;
          task.isBilingualSingle = false;
        } else if (bestBilingual) {
          // Priority B: Bilingual file directly
          task.zh = bestBilingual;
          task.en = null;
          task.isBilingualSingle = true;
        } else if (bestZh) {
          // Priority C: Only Chinese
          task.zh = bestZh;
          task.en = null;
          task.isBilingualSingle = false;
        } else if (bestEn) {
          // Priority D: Only English
          task.zh = null;
          task.en = bestEn;
          task.isBilingualSingle = false;
        } else {
          // Fallback to any normal file
  ...
  shuffleBackdrop: () => {
    ...
    if (tmdbBackdropList.length <= 1) {
      get().addLog("当前影视没有其他备用剧照可选", "info");
      return;
    }
    // Filter out current backdrop to make sure we switch to a different one
